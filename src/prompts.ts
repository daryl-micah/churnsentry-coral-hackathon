  ...
  const schema = `
{
  ...
  const sections = [
    ...
    `# GitHub issues mentioning the customer (engineering view of customer pain)\n${JSON.stringify(data.githubIssues, null, 2)}`,
...
- Provide exactly 3 specific signals that support the diagnosis. Each signal's "source" must name the actual upstream system: "PostHog", "GitHub", "Stripe", or "Slack".
- Recommend 1 immediate action and 1 long-term fix.
- Flag if this looks systemic (same error/complaint pattern observed in PostHog or other GitHub issues).
